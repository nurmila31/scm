	 ...
	 */ 
	 class view {
		...
		function __construct($db){
			...
			function beli_row(){
				$sql ="SELECT SUM(jumlah) as stok FROM faktur";
				$row = $this-> db -> prepare($sql);
				$row -> execute();
				$hasil = $row -> fetch();
				return $hasil;
			...
			function beli(){
				$sql ="SELECT faktur.* , barang.id_barang, barang.nama_barang,barang.merk, barang.harga_beli, supplier.id_supplier,
						supplier.nm_supplier from faktur
					   left join barang on barang.id_barang=faktur.id_barang 
					   left join supplier on supplier.id_supplier=faktur.id_supplier
					   where faktur.periode = ?
					   ORDER BY id_faktur DESC";
				$row = $this-> db -> prepare($sql);
				$row -> execute(array(date('m-Y')));
				$hasil = $row -> fetchAll();
				return $hasil;
			...
			function periode_beli($periode){
				$sql ="SELECT faktur.* , barang.id_barang, barang.nama_barang,barang.merk, barang.harga_beli, supplier.id_supplier,
						supplier.nm_supplier from faktur
					   left join barang on barang.id_barang=faktur.id_barang 
					   left join supplier on supplier.id_supplier=faktur.id_supplier WHERE faktur.periode LIKE ? 
			...
			function hari_beli($hari){
				...
				$sql ="SELECT faktur.* , barang.id_barang, barang.nama_barang,barang.merk,  barang.harga_beli, supplier.id_supplier,
						supplier.nm_supplier from faktur 
					   left join barang on barang.id_barang=faktur.id_barang 
					   left join supplier on supplier.id_supplier=faktur.id_supplier WHERE faktur.tanggal_input LIKE ? 
					   ORDER BY id_faktur ASC";
				$row = $this-> db -> prepare($sql);
				$row -> execute(array($param));
				$hasil = $row -> fetchAll();
			...
			function pembelian(){
				$sql ="SELECT pembelian.* , barang.id_barang, barang.nama_barang, barang.merk, barang.satuan_barang, supplier.id_supplier,
						supplier.nm_supplier from pembelian 
			...
			function jumlah_faktur(){
				$sql ="SELECT SUM(total) as bayar FROM faktur";
				$row = $this -> db -> prepare($sql);
				$row -> execute();
				$hasil = $row -> fetch();
				return $hasil;
